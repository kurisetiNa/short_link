...
public interface LinkAccessLogsMapper extends BaseMapper<LinkAccessLogsDO> {
    ...
    @Select("<script>SELECT ip, COUNT(*) `count` FROM t_link_access_logs " +
            "WHERE gid = #{param.gid} " +
            ...
            "GROUP BY ip ORDER BY `count` DESC LIMIT 5</script>")
    ...
    @Select("<script>SELECT " +
            ...
            "<if test=\"param.fullShortUrl != null and param.fullShortUrl != ''\">" +
            "AND full_short_url = #{param.fullShortUrl} </if>" +
            "GROUP BY `user`) access_users</script>")
    ...
    @Select("<script>" +
            ...
            "    `user`, " +
            "    CASE " +
            "        WHEN DATE(MIN(create_time)) BETWEEN #{param.startDate} AND #{param.endDate} " +
            "        THEN '新访客' " +
            "        ELSE '老访客' " +
            "    END AS uvType " +
            ...
            "GROUP BY `user`" +
            "</script>")
    List<Map<String, Object>> selectUvTypeByUsers(@Param("param") ShortLinkStatsReqDTO requestParam,
                                                   @Param("users") List<String> userAccessLogsList);
}
